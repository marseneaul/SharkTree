...
export class TimeTrack extends BaseTrack {
    ...
    drawTimeConfig(time: TimeConfig, i: number): void {
        ...
        console.log(geologicalName)
        const timeRange = time.timeRange;
        let timeBegin = parseInt(timeRange.split('-')[0].split('(')[0]);
        let timeEnd = parseInt(timeRange.split('-')[1].split('(')[0]);
        const timeBeginUnit = timeRange.split('-')[0].split('(')[1].split(')')[0];
        const timeEndUnit = timeRange.split('-')[1].split('(')[1].split(')')[0];
